...
class ReconstructionDecoder(nn.Module):
    """Decoder for image reconstruction with proper skip connection handling."""
    ...
    def __init__(self,
        ...
        # Skip connection projection layers to handle varying channel dimensions
        # Skip features from ConvNeXt (reverse order for decoder): [384, 192, 96, 96]
        self.skip_projections = nn.ModuleList([
            nn.Conv2d(384, 256, 1),  # For stage 1: skip_features[-1] = 384 -> 256
            nn.Conv2d(192, 64, 1),   # For stage 2: skip_features[-2] = 192 -> 64
            nn.Conv2d(96, 32, 1),    # For stage 3: skip_features[-3] = 96 -> 32
            nn.Conv2d(96, 128, 1),   # For stage 4: skip_features[-4] = 96 -> 128
        ])

        # Decoder stages: 8x8 -> 16x16 -> 32x32 -> 64x64 -> 128x128 -> 256x256
        self.decoder_stages = nn.ModuleList()

        # Stage 0: 512 -> 256 (8x8 -> 16x16)
        self.decoder_stages.append(nn.Sequential(
            nn.ConvTranspose2d(latent_dim, 256, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            *[ResidualBlock(256) for _ in range(depths[0])]
        ))

        # Stage 1: 256+256 -> 128 (16x16 -> 32x32) with skip connection
        self.decoder_stages.append(nn.Sequential(
            nn.Conv2d(512, 256, 1),  # Reduce concatenated channels
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            *[ResidualBlock(128) for _ in range(depths[1])]
        ))

        # Stage 2: 128+64 -> 64 (32x32 -> 64x64) with skip connection
        self.decoder_stages.append(nn.Sequential(
            nn.Conv2d(192, 128, 1),  # Reduce concatenated channels
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            *[ResidualBlock(64) for _ in range(depths[2])]
        ))

        # Stage 3: 64+32 -> 32 (64x64 -> 128x128) with skip connection
        self.decoder_stages.append(nn.Sequential(
            nn.Conv2d(96, 64, 1),   # Reduce concatenated channels
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(64, 32, kernel_size=4, stride=2, padding=1),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            *[ResidualBlock(32) for _ in range(depths[3])]
        ))

        # Final upscaling: 32+128 -> 256x256
        self.final_upscale = nn.Sequential(
            nn.Conv2d(160, 128, 1),   # Reduce concatenated channels
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
    ...
    def forward(self, latent, skip_features=None):
        ...
        # Stage 0: Initial upsampling (no skip connection)
        x = self.decoder_stages[0](x)
        # print(f"Stage 0 output: {x.shape}")  # [B, 256, 16, 16]

        # Stage 1-3: Upsampling with skip connections
        for i in range(1, 4):
            if self.skip_connections and skip_features and len(skip_features) >= i:
                skip_idx = -(i)  # -1, -2, -3
                skip = skip_features[skip_idx]

                # Project skip features to correct dimensions
                if i-1 < len(self.skip_projections):
                    skip = self.skip_projections[i-1](skip)

                # Resize skip to match current resolution
                current_size = x.shape[-2:]
                skip = F.interpolate(skip, size=current_size, mode='bilinear', align_corners=False)

                # Concatenate and apply stage
                x = torch.cat([x, skip], dim=1)
                # print(f"Stage {i} input (after concat): {x.shape}")

            x = self.decoder_stages[i](x)
            # print(f"Stage {i} output: {x.shape}")

        # Stage 4: Final upsampling with remaining skip connection
        if self.skip_connections and skip_features and len(skip_features) >= 4:
            skip = skip_features[-4]  # First skip feature (96 channels)
            skip = self.skip_projections[3](skip)  # Use correct projection: 96 -> 128
            skip = F.interpolate(skip, size=x.shape[-2:], mode='bilinear', align_corners=False)
            x = torch.cat([x, skip], dim=1)
            # print(f"Final stage input (after concat): {x.shape}")

        # Final upscaling to 256x256
        output = self.final_upscale(x)
...
class GenConViTExpert(nn.Module):
    ...
    def _build_model(self):
        ...
        # Add dimension projection for decoder compatibility
        # This bridges the gap between global features and decoder input expectations
        self.feature_projection = nn.Sequential(
            nn.AdaptiveAvgPool2d(8),  # Ensure 8x8 spatial size
            nn.Conv2d(global_feature_dim, self.config.latent_dim, 1),
            nn.BatchNorm2d(self.config.latent_dim),
            nn.ReLU(inplace=True)
        )

        # Reconstruction decoder with correct input dimensions
        self.decoder = ReconstructionDecoder(
            latent_dim=self.config.latent_dim,  # Now matches projected features
            output_channels=3,
    ...
    def reconstruct_image(self, latent: torch.Tensor, skip_features: Optional[List[torch.Tensor]] = None) -> torch.Tensor:
        ...
        # For VAE variant, latent comes from VAE bottleneck (already 1D)
        # For ED variant, we need to project global features to spatial representation
        if len(latent.shape) == 2:
            # Project 1D latent to 2D spatial features for decoder
            batch_size = latent.shape[0]
            # Create spatial features from latent
            latent = latent.view(batch_size, self.config.latent_dim, 1, 1)
            # Expand to 8x8 spatial size to match decoder expectations
            latent = latent.expand(-1, -1, 8, 8)

        reconstructed = self.decoder(latent, skip_features)
        return reconstructed

    def forward(self, images: torch.Tensor, return_reconstruction: bool = False, return_latent: bool = False) -> Dict[str, torch.Tensor]:
        """
        ...
        Args:
            ...
            return_latent: Whether to return latent representation (for VAE)

        Returns:
            Dictionary with 'classification' and optionally 'reconstruction' keys
        """
        batch_size = images.shape[0]
        device = images.device

        # Mark CUDA Graph step boundary to prevent tensor overwriting issues
        # This ensures each forward pass gets fresh tensor memory allocation
        ...
        output_dict = {'classification': logits}

        # Add reconstruction if requested
        if return_reconstruction:
            # FIXED: Handle tensor cloning outside of CUDAGraphs context
            # Clone global_features BEFORE feature_projection to prevent overwriting
            try:
                # Create a detached clone that won't be affected by CUDAGraphs memory reuse
                global_features_detached = global_features.detach().clone()

                # Use feature projection to create decoder-compatible features
                projected_features = self.feature_projection(global_features_detached)

                # Reconstruct using the safely cloned and projected features
                reconstructed = self.decoder(projected_features, skip_features if self.config.skip_connections else None)
                output_dict['reconstruction'] = reconstructed

            except Exception as e:
                # Fallback: try without cloning if CUDA Graphs still cause issues
                if hasattr(torch.compiler, 'cudagraph_mark_step_begin'):
                    torch.compiler.cudagraph_mark_step_begin()

                # Use original global_features directly (may be slower but safer)
                projected_features = self.feature_projection(global_features)
                reconstructed = self.decoder(projected_features, skip_features if self.config.skip_connections else None)
                output_dict['reconstruction'] = reconstructed
                ...
                logger.warning(f"CUDA Graphs tensor handling fallback activated: {e}")

        # Add VAE latent variables if requested
        if return_latent and self.config.variant == GenConViTVariant.VAE:
            if mu is not None:
                output_dict['mu'] = mu
    ...
    def predict(self, images: torch.Tensor, return_features: bool = True) -> ExpertOutput:
        ...
        # FIXED: Handle tensor cloning outside of CUDAGraphs context for reconstruction
        try:
            # Create a detached clone that won't be affected by CUDAGraphs memory reuse
            global_features_detached = global_features.detach().clone()

            # Reconstruction using feature projection with safely cloned tensor
            projected_features = self.feature_projection(global_features_detached)
            reconstructed = self.decoder(projected_features, skip_features if self.config.skip_connections else None)
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"CUDA Graphs tensor handling fallback in predict(): {e}")
