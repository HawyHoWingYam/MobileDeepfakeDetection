...
def _balanced_sample(df: pd.DataFrame, target: int, seed: int) -> pd.DataFrame:
    """Sample up to `target` rows trying to balance real/fake labels."""
    ...
    target_fake = min(len(fake_df), target // 2)
    target_real = min(len(real_df), target - target_fake)

    remaining = target - (target_fake + target_real)
    if remaining > 0:
        avail_fake = max(len(fake_df) - target_fake, 0)
        add_fake = min(remaining, avail_fake)
        target_fake += add_fake
        remaining -= add_fake

    if remaining > 0:
        avail_real = max(len(real_df) - target_real, 0)
        add_real = min(remaining, avail_real)
        target_real += add_real
        remaining -= add_real

    if target_fake > len(fake_df):
        target_fake = len(fake_df)
    if target_real > len(real_df):
        target_real = len(real_df)

    sampled_real = (
        real_df.sample(n=target_real, random_state=rng_real, replace=False)
        if target_real > 0 else real_df.head(0)
    )
    sampled_fake = (
        fake_df.sample(n=target_fake, random_state=rng_fake, replace=False)
        if target_fake > 0 else fake_df.head(0)
    )

    sampled = pd.concat([sampled_real, sampled_fake], ignore_index=True)
    return sampled


def _build_mixed_manifest(
    df_difficult: pd.DataFrame,
    ...
    if not dataset_full_records:
        logger.warning("Cannot build mixed manifest: no dataset predictions available.")
        return None

    all_records = pd.concat(dataset_full_records.values(), ignore_index=True)
    all_records = all_records.drop_duplicates(subset=["dataset", "image_path"], keep="first")

    total_count = int(round(len(all_records) * total_fraction))
    if total_count <= 0:
        logger.warning(
            "Requested mixed manifest fraction yielded zero samples (total_fraction=%.3f).",
            total_fraction,
        )
        return None

    difficult_target = int(round(total_count * difficult_fraction))
    easy_target = int(round(total_count * easy_fraction))

    difficult_target = min(difficult_target, len(df_difficult))
    if difficult_target <= 0 and not df_difficult.empty:
        difficult_target = min(len(df_difficult), total_count)

    difficult_selected = (
        _balanced_sample(df_difficult, difficult_target, seed)
        if difficult_target > 0 else df_difficult.head(0)
    )

    easy_pool = all_records[all_records["difficulty"] == "none"].copy()
    if not difficult_selected.empty:
        chosen_keys = set(
            (difficult_selected["dataset"].astype(str) + "|" + difficult_selected["image_path"].astype(str)).tolist()
        )
        easy_pool = easy_pool[
            ~(easy_pool["dataset"].astype(str) + "|" + easy_pool["image_path"].astype(str)).isin(chosen_keys)
        ]

    target_fake_total = total_count // 2
    target_real_total = total_count - target_fake_total

    diff_fake = int((difficult_selected["label"] == 1).sum())
    diff_real = int((difficult_selected["label"] == 0).sum())
    need_fake = max(target_fake_total - diff_fake, 0)
    need_real = max(target_real_total - diff_real, 0)

    easy_target = max(easy_target, need_fake + need_real)
    easy_target = min(easy_target, max(total_count - len(difficult_selected), 0))

    easy_selected = _balanced_sample(easy_pool, easy_target, seed + 101)

    combined = pd.concat([difficult_selected, easy_selected], ignore_index=True)

    if len(combined) > total_count:
        combined = combined.sample(n=total_count, random_state=seed)

    if len(combined) < total_count:
        remaining = total_count - len(combined)
        if remaining > 0:
            remaining_pool = df_difficult[
                ~(df_difficult["dataset"].astype(str) + "|" + df_difficult["image_path"].astype(str)).isin(
                    combined["dataset"].astype(str) + "|" + combined["image_path"].astype(str)
                )
            ]
            extra = _balanced_sample(remaining_pool, remaining, seed + 202)
            combined = pd.concat([combined, extra], ignore_index=True)

    combined = combined.drop_duplicates(subset=["dataset", "image_path"], keep="first")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to write a mixed manifest with difficult/easy samples for Stage 03",
    )
    parser.add_argument(
        "--mix_total_fraction",
        type=float,
        default=0.10,
        help="Total fraction of the original training pool to include in the mixed manifest",
    )
    parser.add_argument(
        "--mix_difficult_fraction",
        type=float,
        default=0.70,
        help="Fraction of the mixed manifest reserved for difficult samples",
    )
    parser.add_argument(
        "--mix_easy_fraction",
        type=float,
        default=0.20,
        help="Minimum fraction of the mixed manifest to fill with easy samples",
    )
    parser.add_argument(
        "--mix_seed",
        type=int,
        default=42,
        help="Random seed for sampling the mixed manifest",
    )
    ...
    mix_manifest_path = args.mix_manifest
    mix_manifest_df: Optional[pd.DataFrame] = None
    if mix_manifest_path:
        mix_manifest_df = _build_mixed_manifest(
            df_difficult=df_difficult,
            dataset_full_records=dataset_full_records,
            total_fraction=max(args.mix_total_fraction, 0.0),
            difficult_fraction=max(args.mix_difficult_fraction, 0.0),
            easy_fraction=max(args.mix_easy_fraction, 0.0),
            seed=int(args.mix_seed),
        )
        if mix_manifest_df is not None and not mix_manifest_df.empty:
            mix_path = Path(mix_manifest_path)
            mix_path.parent.mkdir(parents=True, exist_ok=True)
            mix_manifest_df.to_csv(mix_path, index=False)
            logger.info(
                "Saved mixed manifest: %s (rows=%d, difficult=%d, easy=%d)",
                mix_path,
                len(mix_manifest_df),
                int((mix_manifest_df["difficulty"] != "none").sum()),
                int((mix_manifest_df["difficulty"] == "none").sum()),
            )
        else:
            logger.warning("Requested mix manifest was not generated due to insufficient data.")
    ...
    if mix_manifest_path and mix_manifest_df is not None:
        summary["mixed_manifest"] = {
            "path": str(mix_manifest_path),
            "total_rows": int(len(mix_manifest_df)),
            "difficult_rows": int((mix_manifest_df["difficulty"] != "none").sum()),
            "easy_rows": int((mix_manifest_df["difficulty"] == "none").sum()),
        }
