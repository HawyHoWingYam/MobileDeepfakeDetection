...
class CelebDFDataset(Dataset):
    ...
    def __init__(self,
        ...
        # Filter valid samples (only if 'valid' column exists)
        # Previous implementation used DataFrame.get('valid', True) which returns a scalar True
        # when the column is missing, causing a KeyError when used for boolean indexing.
        if 'valid' in self.df.columns:
