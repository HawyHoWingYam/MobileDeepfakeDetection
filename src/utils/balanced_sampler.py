"""
AWARE-NET: Advanced Balanced Batch Sampler for Contrastive Learning (Research Module)

⚠️  ADVANCED RESEARCH MODULE - NOT NEEDED FOR STAGE 01:
This module implements specialized sampling for supervised contrastive learning.
For basic Stage 01 requirements, standard DataLoader with WeightedRandomSampler is sufficient.

Research Purpose:
- Balanced sampling specifically for contrastive learning
- Ensures sufficient positive/negative pairs per batch
- Advanced sampling strategies for research experiments

Key Features (Research Level):
- Optimized for supervised contrastive learning pairs
- Maintains class balance for stable contrastive training
- Specialized batch construction for research scenarios

Stage 01 Alternative:
For standard classification, use WeightedRandomSampler from train_mobile_simple.py
(which already handles true/fake dataset balancing)
"""
