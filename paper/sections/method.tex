...
\begin{equation}
\pi_2(\tau_{\mathrm{low}},\tau_{\mathrm{high}}) = \Pr(\tau_{\mathrm{low}} \le p_1(x) \le \tau_{\mathrm{high}})
\end{equation}
\label{eq:escalation_rate}
...
\begin{figure}[ht]
  ...
  \begin{itemize}[leftmargin=*,itemsep=8pt]
    \item \textbf{Stage 1: MobileNetV4 filter} --- fast frame-level screening on device
    \item $\downarrow$ escalate ambiguous cases
    \item \textbf{Stage 2: EfficientNetV2 expert} --- high-fidelity logits and embeddings
    \item $\downarrow$ offline threshold and policy search (Stage 4)
    \item \textbf{Stage 4: Cascade tuning} --- choose $(\tau_{\mathrm{low}},\tau_{\mathrm{high}})$ under escalation budget
    \item $\downarrow$ robustness and OOD evaluation (Stage 5)
    \item \textbf{Stage 6: Mobile deployment} --- export calibrated models and thresholds to device
  \end{itemize}
