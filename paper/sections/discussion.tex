...
A key practical question is whether cascade behaviour observed on desktop transfers reliably to mobile deployment. Our on\hyp device evaluation on the \texttt{mobile\_eval} subset (Section~\ref{sec:ondevice_results}) demonstrates that the same threshold configuration ($\tau\_{low}=0.02$, $\tau\_{high}=0.98$, $\tau\_{s2}=0.5$) produces qualitatively similar results across platforms: both PC and mobile achieve low FNR ($<$3\%), confirming that the safety\hyp oriented bias is preserved through ONNX export.
