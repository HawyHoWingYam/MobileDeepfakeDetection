...
Stage~2 training incorporates hard example mining (HEM) to focus learning on samples near the decision boundary. We compare two training strategies: \textbf{Standard} (uniform sampling) vs.\ \textbf{HEM} (prioritize samples with loss $> 75$th percentile). Table~\ref{tab:ablation_hem} reports Stage~2\hyp only performance with and without HEM. The HEM experiments use an auxiliary loss\hyp based sampler that oversamples difficult samples; the released Stage~2 training script in \verb|src/stage2/train_stage2_effnet.py| defaults to the Standard configuration with uniform sampling (HEM disabled).
...
\begin{enumerate}[leftmargin=*,itemsep=2pt,parsep=0pt]
  ...
  \item \textbf{Hard example mining provides consistent gains.} HEM improves Stage~2 performance by +0.76\% F1 and -7.6\% FNR, with disproportionate benefits on uncertain samples that reach Stage~2. The technique is straightforward to implement and imposes minimal training overhead.
