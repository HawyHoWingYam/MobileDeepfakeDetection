...
```
outputs/stage1/
├── run_*/training_summary.json     # 訓練過程與最佳 AUC
├── calibration_temp.json           # 溫度縮放參數（Stage 1）
├── calibration_summary.json        # 校準過程摘要
├── evaluation/stage1_eval_summary.csv  # 各資料集測試指標
├── reliability_diagram.png         # 校準效果圖
└── probability_distributions.png   # 機率分佈可視化
```

### 📊 實際性能基線（對齊論文）
- **驗證集 AUC**：約 0.993（combined validation；見論文 Results）  
- **各測試集 F1**：在 CelebDF-v2 / FF++ / DFDC / DeeperForensics 上均高於 0.90，NeuralTextures 等子集略低但仍在可接受範圍。  
- **ECE（校準後）**：約 0.9% 左右（約 0.0089），顯示經溫度縮放後機率相當可靠。  
- **在最終級聯中**：Stage~1 搭配 Stage~2 並經 Stage~4 閾值調優後，可將整體 FNR 壓到約 0.5–0.6\%，且 Stage~2 使用率約 1–1.5\%。
