...
| 阶段 | 名称 | 关键技术点 |
|------|------|-----------|
| 1 | 预处理 | 人脸检测与对齐、裁剪归一化、数据增强（亮度/对比度/压缩噪声） |
| 2 | Stage1训练 | MobileNetV4、二分类交叉熵、优先优化召回率 |
| 3 | Stage2训练 | EfficientNetV2-B3、聚焦低置信度样本、边界判别能力 |
| 4 | 阈值调优 | 网格搜索τ_low/τ_high、约束FNR与升级率 |
| 5 | 鲁棒性评估 | 不同压缩/分辨率/噪声条件、验证泛化能力 |
| 6 | 移动导出 | INT8量化、ONNX导出、实测延迟180ms/准确率92.8% |

### 视觉元素

- **流水线时间轴**: 六个连续方块从左到右
  - Pre-processing → Stage1 Training → Stage2 Training → Threshold Tuning → Robustness Evaluation → Mobile Export
- 每个方块下用一句话标注关键动作
- 最后一个方块旁边标注关键数字: 180ms / 92.8%

### 讲稿

> 这一页我用一条流水线来概括整个方法从训练到落地的过程，总共分成六个阶段。
>
> 第一个阶段是预处理。我们会对视频帧进行人脸检测与对齐，把人脸区域裁剪出来并统一到固定分辨率，同时加入亮度、对比度、压缩伪影等数据增强，模拟真实移动端采集环境中的各种干扰。
>
> 第二个阶段是Stage1的训练。这里使用大规模的真伪混合数据训练MobileNetV4，采用标准的二分类交叉熵损失。训练时我更关注召回率，确保第一阶段尽量不要漏掉伪造样本。
>
> 第三个阶段是Stage2的训练。与Stage1不同，Stage2更关注那些在Stage1中置信度较低、容易混淆的样本，通过EfficientNetV2-B3对这些边界样本进行更精细的学习。
>
> 第四个阶段是阈值调优。我在验证集上对τ_low和τ_high进行网格搜索，目标是在FNR大约0.6%的前提下，将升级到Stage2的比例控制在约1.2%。
>
> 第五个阶段是鲁棒性评估。我们在不同的压缩质量、分辨率以及噪声条件下测试级联系统，验证方法在实际复杂场景中的稳定性。
>
> 最后是移动导出阶段。对模型进行INT8量化，导出为ONNX格式，在真实Android设备上测量推理延迟和检测准确率。最终系统在保证约92.8%准确率的情况下，将单帧延迟控制在约180毫秒。
...
**模型压缩与量化**
- 对Stage1/Stage2进行INT8后训练量化（post-training quantization）
- 通过校准数据集减少量化误差，保持整体准确率在92.8%左右
- FP32 TorchScript: Stage1 37.3MB / Stage2 49.6MB
- INT8 ONNX: Stage1 ~37.5MB / Stage2 ~52MB

**推理框架与导出**
- 训练端使用PyTorch完成训练
- 导出为ONNX格式，适配移动端推理引擎（ONNX Runtime Mobile / NNAPI）
- 针对ARM架构开启向量化加速（NEON / GPU后端）
...
| 推理层 | 加载INT8 ONNX模型，异步执行Stage1/Stage2推理 |
...
- **手机端分层结构图**:
  ...
  - 底层: Inference Engine（ONNX Runtime / NNAPI）+ 两个模型框
- 底层旁边标出: INT8、180ms、92.8%
- 用不同颜色区分"应用逻辑层"和"模型推理层"

### 讲稿

> 最后这一页介绍MobileDeepfake在移动端的整体架构设计。
>
> 首先是模型压缩与量化。为了能够在手机上以较低的延迟运行两个模型，我对Stage1和Stage2都进行了INT8量化，将权重和部分激活从浮点表示压缩到8比特。在这个过程中，我使用了代表性的数据集进行校准，尽量减少量化带来的精度损失。原始的FP32 TorchScript模型中，Stage1大小约为37.3MB，Stage2约为49.6MB；导出为INT8 ONNX之后，Stage1大约为37.5MB、Stage2约为52MB，在大多数中高端手机上都可以接受。
>
> 在推理框架方面，训练阶段使用PyTorch完成训练和验证，然后将模型导出为ONNX格式。在手机端，通过轻量级的ONNX推理引擎，结合ARM平台的NEON指令集和必要时的GPU加速，实现对INT8模型的高效推理。
>
> Android集成方面，整体采用分层架构。最上层是应用层，负责相机预览和用户界面展示。中间层首先对图像做预处理，包括人脸检测与裁剪、缩放归一化等。底层则是推理层，它会加载两个已经量化的ONNX模型，对每一帧首先运行Stage1推理，如果置信度落入双阈值中间区域，再异步调用Stage2进行二次判断。
>
...
- **左侧**: 2×2表格，列出4个训练数据集的规模与特点
- **右侧**: 世界地图热力图或带旗帜的小图标，标示Deepfake-Eval-2024的多语言来源
- 用图标表示"88网站""52语言""2024媒体"等关键数字

### 讲稿

> 这一部分先介绍一下本工作的数据集构成。整体上，我们一共使用了大约三百五十万张人脸裁剪图像，覆盖四个主流深度伪造数据集，以及一个面向现实世界场景的OOD测试集。
>
...
>
> 在测试阶段，我们还引入了Deepfake-Eval-2024数据集作为分布外测试集。它来自88个网站、52种语言，并且是2024年真实媒体环境中采集的内容，能更好地反映当前互联网上深度伪造的真实分布。
...
- **级联整体表现**:
  - 最终FNR ≈ 0.6%（漏检率极低）
  - 升级率 ≈ 1.2%（只有少量样本需要进入Stage2）
- **结论**: 在公开基准数据上，级联结构既保证了高检测能力，又极大节省计算

### 视觉元素

- **左图**: Stage1与Stage2的ROC曲线对比图，曲线旁标注各自AUC数值
- **右图**: 条形图或表格展示: Stage1 AUC、Stage2 AUC、级联FNR、升级率
- 标注"FNR ~0.6%"、"升级率 ~1.2%"等关键数字

### 讲稿

> 这个页面展示的是在in-domain，也就是在训练分布相同或相近的测试集上的结果。左侧是Stage1和Stage2的ROC曲线，可以看到Stage1的AUC达到0.9936，Stage2也有0.9633。
>
> 在设计上，Stage1主要目标是高召回、快速筛查，它用一个相对轻量的模型捕捉大部分伪造样本；Stage2则更精细，用来处理边界样本。
>
> 整体级联之后，如右图所示，最终的漏检率FNR只有大约0.6%，而需要进入第二阶段的升级率只有大约1.2%。这意味着在公开基准数据集上，我们既能保持高精度的检测能力，又只对极少数难样本使用较重的计算。
...
**实际运行统计（In-domain）**
- 升级率 ≈ 1.2% → 约98.8%样本只经过Stage1
- 在保持FNR ≈ 0.6%的前提下，整体计算量显著降低
...
- **对比示意**: "单阶段重模型" vs "两阶段级联" 的平均计算量对比

### 讲稿

> 为了更好地适配移动端和大规模部署，我们采用了两阶段级联的结构。Stage1是一个轻量、高吞吐的检测器，它的原则是宁可多抓一点嫌疑样本，也要尽量减少漏检。
>
> Stage2则相对较重，主要针对Stage1判定不确定的难样本做精细分析。右侧漏斗图可以看到，在in-domain场景下，只有大约1.2%的样本需要被升级到Stage2，大概98.8%的样本在Stage1就已经完成了判决。
>
> 这样设计的好处是，在保持整体漏检率大约0.6%的前提下，我们显著降低了平均计算成本。这为后面在移动端、甚至是端云协同的部署，提供了一个比较现实的技术基础。
...
- **对比条形图**: 横轴In-domain vs Deepfake-Eval-2024，纵轴F1/FNR/升级率
- 用醒目颜色强调F1大幅下降、FNR大幅上升

### 讲稿

> 但是，当我们把同样的模型直接应用到Deepfake-Eval-2024上时，情况就完全不一样了。右边的对比图可以看到，和in-domain相比，性能出现了明显下降。
>
> 具体来说，在这个OOD数据集上，我们的F1值低于0.30，漏检率FNR大约在73%到79%之间，也就是说有大量伪造样本被误判为真实。同时，Stage2的升级率达到大约51%，意味着有一半的样本被模型认为是'困难样本'。
>
...
- **折线图2**: 噪声/模糊强度 vs F1，表现为平缓下降
- **示例图**: 高质量伪造、极度压缩、遮挡严重的失败样本截图

### 讲稿

> 最后这一页，我们简单总结一下鲁棒性实验和错误分析。我们主要从图像扰动和典型失败案例两个角度来观察模型的行为。
>
> 在图像扰动方面，如左图所示，当我们对图像做JPEG压缩时，发现中等程度的压缩反而可以在部分场景下提升F1。一个可能的原因是，适度压缩会抑制一些随机高频噪声，让伪造边界和不自然的纹理更突出。
>
> 对于运动模糊，随着模糊程度增加，性能会逐渐下降，但整体是一个平滑的过程，没有出现突然完全失效的情况。
>
...
**模型结构**
| 阶段 | 模型 | FP32 TorchScript | INT8 ONNX |
|------|------|------------------|-----------|
...
- 推理框架: ONNX Runtime Mobile（裁剪无用算子，减小包体积）
- 前后处理: 人脸检测/对齐在本地实现，多线程流水线
...
>
> 在模型设计上，我们采用两阶段级联结构：Stage1使用轻量的MobileNetV4，FP32 TorchScript约37.3MB；Stage2使用更强的EfficientNetV2-B3，FP32约49.6MB。导出为INT8 ONNX之后，Stage1大约37.5MB、Stage2约52MB，前者作为快速过滤器用于筛查明显的真实样本，后者作为专家模型专门处理风险较高的样本。
>
> 为了适应移动端资源限制，两阶段模型都进行了INT8后训练量化，并统一导出为ONNX格式；推理框架选用了ONNX Runtime Mobile，通过裁剪无用算子，显著减小了运行库的体积。在推理部署上，目前在Xiaomi 13上主要采用CPU基线实现；GPU和NNAPI的进一步加速留作后续工程优化方向。
>
> 在前后处理方面，我们将人脸检测、裁剪和简单对齐也放在本地实现，并与推理线程进行流水线并行。整个工程从训练脚本到ONNX导出，再到APP集成，都做成了可复现的自动化流水线。
