...
\subtitle{A Cascaded Detection System for On-Device Deepfake Detection}
\author{Wing Yam Ho}
...
\begin{center}
\begin{tabular}{>{\raggedright}p{3.5cm} p{8cm}}
\toprule
...
\midrule
0 & Preprocessing & Face detection, alignment, cropping, augmentation \\
1 & Stage1 Training & MobileNetV4, BCE loss, recall optimization \\
2/3 & Stage2 Training & EfficientNetV2-B3, boundary sample focus \\
4 & Threshold Tuning & Grid search for $\tau_{\text{low}}/\tau_{\text{high}}$, FNR constraint \\
5 & Robustness & Compression, resolution, noise testing \\
6 & Mobile Export & INT8 quantization, ONNX, \highlight{180ms / 92.8\%} \\
\bottomrule
\end{tabular}
\end{center}
\end{frame}
...
\setlength\itemsep{0.1em}
    ...
    \item Calibration dataset for accuracy
    \item Stage1: 37.3MB / Stage2: 49.6MB {\scriptsize (FP32 TorchScript)}
    \item INT8 ONNX: $\sim$37.5MB / $\sim$52MB
\end{itemize}
...
\setlength\itemsep{0.1em}
    ...
    \item ONNX Runtime Mobile / NNAPI
    \item ARM NEON vectorization
\end{itemize}
...
\begin{columns}[T]
\begin{column}{0.55\textwidth}
\textbf{Training Datasets ($\sim$3.5M face crops)}
...
\begin{column}{0.45\textwidth}
\begin{block}{OOD Test: Deepfake-Eval-2024}
\begin{itemize}
    \item Val: 451,917 / Test: 402,413
    \item \highlight{88 websites}
    \item \highlight{52 languages}
    \item 2024 real media environment
    \item Multi-platform, multi-language
\end{itemize}
\end{block}
...
\begin{itemize}
    ...
    \item Official train/val/test splits
\end{itemize}
...
\midrule
In-domain & AUC, Cascade FNR, Escalation Rate \\
OOD & F1, FNR, Stage2 Escalation Rate \\
\bottomrule
\end{tabular}
\end{center}
\end{column}
...
\node[box] (c2) at (1.8,3) {FF++};
\node[box] (c3) at (0,2) {DFDC};
\node[box] (c4) at (1.8,2) {Deeper\\Forensics};

% Model
\node[box, fill=secondaryblue!30, minimum width=3.5cm] (model) at (0.9,0.5) {Cascade Model};

% Tests
\node[box, fill=successgreen!20] (in) at (-1,-1) {In-domain\\Test};
\node[box, fill=alertred!20] (ood) at (2.8,-1) {OOD Test\\(No Finetune)};
...
\vspace{0.5em}
\begin{block}{Goal}
Simulate: Train on public data $\rightarrow$ Deploy to real internet
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
    \item Stage 1 AUC: \highlight{0.9936}
    \item Stage 2 AUC: 0.9633
\end{itemize}

\vspace{0.5em}
\textbf{Cascade Performance}
\begin{itemize}
    \item Final FNR: \successtext{$\sim$0.6\%}
    \item Stage 2 Escalation: \successtext{$\sim$1.2\%}
\end{itemize}

\vspace{1em}
\begin{block}{Conclusion}
On public benchmarks, cascade achieves high detection with significant compute savings.
\end{block}
\end{column}
...
\textbf{Runtime Statistics (In-domain)}
\begin{itemize}
\setlength\itemsep{0.1em}
    \item Escalation rate: $\sim$1.2\%
    \item $\Rightarrow$ \highlight{98.8\%} samples only need Stage 1
    \item FNR maintained at $\sim$0.6\%
\end{itemize}

\textbf{Significance for Mobile/Edge}
\begin{itemize}
\setlength\itemsep{0.1em}
    \item Most samples: fast local detection
    \item Few samples: optional cloud refinement
\end{itemize}
\end{column}
...
\begin{figure}
    ...
    \caption*{F1 Score Heatmap ($\tau_{\text{low}}$ vs $\tau_{\text{high}}$)}
\end{figure}

\begin{tikzpicture}[scale=0.6]
    % Funnel
    \fill[primaryblue!30] (-2,2) -- (2,2) -- (1.5,1) -- (-1.5,1) -- cycle;
    \fill[primaryblue!50] (-1.5,1) -- (1.5,1) -- (0.3,0) -- (-0.3,0) -- cycle;

    \node at (0,1.5) {\scriptsize 100\% Input};
    \node at (0,0.5) {\scriptsize 1.2\% Stage2};
\end{tikzpicture}
\end{column}
\end{columns}
\end{frame}
...
Motion Blur & Gradual decline, no catastrophic failure \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{llcc}
\toprule
\textbf{Stage} & \textbf{Model} & \textbf{FP32} & \textbf{INT8 ONNX} \\
\midrule
...
\begin{itemize}
    ...
    \item ONNX Runtime Mobile (trimmed)
    \item Multi-thread pipeline
\end{itemize}
\end{column}
...
\begin{itemize}
    ...
    \item Training to APP: automated packaging
...
\begin{itemize}
    \item Supervisor: [Supervisor Name]
    \item Research Group Members
    \item Open-source communities (ONNX Runtime, PyTorch)
    \item Public dataset contributors
\end{itemize}

\vspace{2em}
{\small This work provides a baseline platform for on-device deepfake detection research.}
\end{center}
\end{frame}
