...
Bridge the gap between \highlight{research prototype} and \highlight{auditable mobile deployment} with explicit, reproducible controls.
\end{block}
\end{frame}
...
\vspace{0.3em}
\begin{block}{Key Insight}
Cascade achieves \successtext{5$\times$ lower FNR} than Stage 1 alone with only \highlight{9\%} compute overhead.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
Training manifests are \highlight{balanced by class and dataset} to ensure fair multi-source learning.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
{\scriptsize \textit{Headline metrics; full tables include Acc, Precision/Recall, FPR}}
\end{column}
...
\begin{itemize}
\setlength\itemsep{0.1em}
    \item Stage 1 alone: Fast but higher FNR
    \item Stage 2 alone: Lower FNR but 5$\times$ slower
    \item Cascade: Best of both worlds
\end{itemize}

\vspace{0.3em}
\textbf{Stage-wise Comparison (Combined Val)}
\begin{center}
\scriptsize
\begin{tabular}{lrrrr}
\toprule
...
\begin{itemize}
\setlength\itemsep{0.1em}
    \item Stage 1: 0.54 GFLOPs
    \item Stage 2: 2.87 GFLOPs (5.3$\times$)
    \item Cascade avg: 0.59 GFLOPs
    \item Overhead: only 9\% vs Stage 1
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\textbf{Runtime Statistics (Combined Validation)}
\begin{itemize}
\setlength\itemsep{0.1em}
...
\vspace{0.3em}
\begin{block}{Key Insight}
Mobile deployment trades \alerttext{4\% accuracy} for \highlight{on-device privacy} and \highlight{$<$200ms latency}.
\end{block}
\end{column}
\end{columns}
\end{frame}
