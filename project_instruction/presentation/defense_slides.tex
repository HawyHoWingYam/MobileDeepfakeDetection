...
Existing cascade systems rarely provide explicit FNR control with auditable cost-risk trade-offs for mobile.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
\setlength\itemsep{0.02em}
    \item Stage 1: Fast but FNR = 3.12\%
    \item Stage 2: Higher FNR (9.43\%), different error modes
    \item Cascade: Complementary $\rightarrow$ FNR = 0.60\%
\end{itemize}
...
\midrule
\textbf{Cascade} & \textbf{0.9941} & \textbf{0.9580} & \successtext{0.60\%} & \highlight{0.59} \\
\bottomrule
\end{tabular}
\end{center}
